...
def get_value(img):
    ...
    file_list=open("char_data/china_list.txt","r")
    ...
    while file_name:
        ...
        data_list=open("char_data/"+file_name+"/data_list.txt","r",encoding="utf-8")
        ...
        while data_name:
            ...
            #权值，对比7，特征3
            if oci:
                per=(1-var[-1]/(final.shape[0]*final.shape[1]))*0.8+((origin_value+curve_value+intersect_value)/100)*0.2
            else:
...
while char_x_up<=row:
    ...
    while char_y_left<=col:
        ...
        if char_x_up<row-1 and char_y_left<col-1:
            ...
            #得到值
            #如果只有一行，加一行
            if img_temp[img_x_up:img_x_down,:].ndim==1:
                char_value=get_value(np.vstack((img_temp[img_x_up:img_x_down,:],img_temp[img_x_up:img_x_down,:])))
            else:
